...
maximum amplicon length and the maximum number of threads.
#+end_export
...
#+begin_src go <<Run the analysis>>=
  ...
  //  var mu sync.Mutex
    for _, genomePath := range genomePaths {
	    wg.Add(1)
	    go processGenome(&wg, genomePath, primerRegexps, *optL)
    }
    wg.Wait()
#+end_src
#+begin_export latex
...
#+begin_src go <<Functions>>=
  func processGenome(wg *sync.WaitGroup, genomePath string,
	  primerRegexps [4]*regexp.Regexp, ampL int) {
	  defer wg.Done()
	  //<<Read the genome>>
	  //<<Find amplicons>>
  }
...
be upright by default. and try to find all matches for the forward
